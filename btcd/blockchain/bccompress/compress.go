...
func IsPubKeyHash(script []byte) (bool, []byte) {
	if len(script) == 25 && script[0] == txsparser.OP_DUP &&
		script[1] == txsparser.OP_HASH160 &&
		script[2] == txsparser.OP_DATA_20 &&
		script[23] == txsparser.OP_EQUALVERIFY &&
		script[24] == txsparser.OP_CHECKSIG {
...
func IsScriptHash(script []byte) (bool, []byte) {
	if len(script) == 23 && script[0] == txsparser.OP_HASH160 &&
		script[1] == txsparser.OP_DATA_20 &&
		script[22] == txsparser.OP_EQUAL {
...
func IsPubKey(script []byte) (bool, []byte) {
	...
	if len(script) == 35 && script[0] == txsparser.OP_DATA_33 &&
		script[34] == txsparser.OP_CHECKSIG && (script[1] == 0x02 ||
		script[1] == 0x03) {
	...
	if len(script) == 67 && script[0] == txsparser.OP_DATA_65 &&
		script[66] == txsparser.OP_CHECKSIG && script[1] == 0x04 {
...
func DecompressScript(compressedPkScript []byte) []byte {
	...
	case cstPayToPubKeyHash:
		...
		pkScript[0] = txsparser.OP_DUP
		pkScript[1] = txsparser.OP_HASH160
		pkScript[2] = txsparser.OP_DATA_20
		copy(pkScript[3:], compressedPkScript[bytesRead:bytesRead+20])
		pkScript[23] = txsparser.OP_EQUALVERIFY
		pkScript[24] = txsparser.OP_CHECKSIG
		return pkScript
	...
	case cstPayToScriptHash:
		...
		pkScript[0] = txsparser.OP_HASH160
		pkScript[1] = txsparser.OP_DATA_20
		copy(pkScript[2:], compressedPkScript[bytesRead:bytesRead+20])
		pkScript[22] = txsparser.OP_EQUAL
		return pkScript
	...
	case cstPayToPubKeyUncomp4, cstPayToPubKeyUncomp5:
		...
		pkScript[0] = txsparser.OP_DATA_65
		copy(pkScript[1:], key.SerializeUncompressed())
		pkScript[66] = txsparser.OP_CHECKSIG
		return pkScript
	}
