...
//	log.Infof("ProcessBlock: ChainLock.RLock")
	...
	/*
	if block.MsgBlock().Header.Nonce < -1 && block.MsgBlock().Header.Timestamp.Unix() < prevNode.Data.TimeStamp() {
		// it is generated by the same committe as the previous one, timestamp should be strickly increasing
