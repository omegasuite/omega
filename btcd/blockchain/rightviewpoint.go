...
func (entry * RightEntry) Father() chainhash.Hash {
	return entry.father
}

func (entry * RightEntry) Desc() []byte {
	return entry.desc
}

func (entry * RightEntry) Attrib() uint8 {
	return entry.attrib
}
...
type RightViewpoint struct {
	entries  map[chainhash.Hash]*RightEntry
...
func (view * RightViewpoint) LookupEntry(p chainhash.Hash) * RightEntry {
	return view.entries[p]
}

// addVertex adds the specified vertex to the view.
func (view * RightViewpoint) addRight(b *wire.RightDef) {
	h := b.Hash()
...
func (view * RightViewpoint) AddRights(tx *btcutil.Tx) {
	...
	for _, txVtx := range tx.MsgTx().TxDef {
		switch txVtx.(type) {
			case *wire.RightDef:
				view.addRight(txVtx.(*wire.RightDef))
			break
		}
	}
}
...
// to the passed block.
func (view * RightViewpoint) connectTransactions(block *btcutil.Block) error {
...
func (view * RightViewpoint) fetchEntry(db database.DB, hash *chainhash.Hash) (*RightEntry, error) {
...
func (view * RightViewpoint) Entries() map[chainhash.Hash]*RightEntry {
	return view.entries
}
...
func NewRightViewpoint() * RightViewpoint {
	return &RightViewpoint{
		entries: make(map[chainhash.Hash]*RightEntry),
	}
}
...
// any) is NOT.
func (b *BlockChain) FetchRightEntry(hash chainhash.Hash) (*RightEntry, error) {
	b.chainLock.RLock()
	defer b.chainLock.RUnlock()

	var entry *RightEntry
