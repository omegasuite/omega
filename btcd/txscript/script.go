...
func IsPayToScriptHash(script []byte) bool {
	...
	return txsparser.IsScriptHash(pops)
}

func parsedOpcode2scriptInfo(pops []txsparser.ParsedOpcode) []scriptInfo {
	newscripts := make([]scriptInfo, len(pops))
	for i,p := range pops {
		newscripts[i] = scriptInfo{p, opcodeFuncs[p.Opcode.Value]}
	}
	return newscripts
}

func scriptInfo2parsedOpcode(pops []scriptInfo) []txsparser.ParsedOpcode {
	newscripts := make([]txsparser.ParsedOpcode, len(pops))
	for i,p := range pops {
		newscripts[i] = p.ParsedOpcode
	}
	return newscripts
}
...
func IsPayToWitnessScriptHash(script []byte) bool {
	...
	return txsparser.IsWitnessScriptHash(pops)
}
...
func IsPayToWitnessPubKeyHash(script []byte) bool {
	...
	return txsparser.IsWitnessPubKeyHash(pops)
}
...
func IsWitnessProgram(script []byte) bool {
	...
	return isWitnessProgram(parsedOpcode2scriptInfo(pops))
}
...
// bytes.
func isWitnessProgram(pops []scriptInfo) bool {
	return len(pops) == 2 &&
		txsparser.IsSmallInt(pops[0].Opcode) &&
		canonicalPush(pops[1].ParsedOpcode) &&
...
func isPushOnly(pops []scriptInfo) bool {
...
func IsPushOnlyScript(script []byte) bool {
	...
	return isPushOnly(parsedOpcode2scriptInfo(pops))
}

// unparseScript reversed the action of parseScript and returns the
// parsedOpcodes as a list of bytes
func unparseScript(pops []txsparser.ParsedOpcode) ([]byte, error) {
...
func DisasmString(buf []byte) (string, error) {
	...
	opcodes, err := txsparser.ParseScript(buf)
...
// stream in pkscript
func removeOpcode(pkscript []txsparser.ParsedOpcode, opcode byte) []txsparser.ParsedOpcode {
	retScript := make([]txsparser.ParsedOpcode, 0, len(pkscript))
...
func scriptCanonicalPush(pop scriptInfo) bool {
	opcode := pop.Opcode.Value
	data := pop.Data
	dataLen := len(pop.Data)
	if opcode > txsparser.OP_16 {
		return true
	}

	if opcode < txsparser.OP_PUSHDATA1 && opcode > txsparser.OP_0 && (dataLen == 1 && data[0] <= 16) {
		return false
	}
	if opcode == txsparser.OP_PUSHDATA1 && dataLen < txsparser.OP_PUSHDATA1 {
		return false
	}
	if opcode == txsparser.OP_PUSHDATA2 && dataLen <= 0xff {
		return false
	}
	if opcode == txsparser.OP_PUSHDATA4 && dataLen <= 0xffff {
		return false
	}
	return true
}
...
// or the push instruction contained wherein is matches the canonical form
// or using the smallest instruction to do the job. False otherwise.
func canonicalPush(pop txsparser.ParsedOpcode) bool {
	opcode := pop.Opcode.Value
	data := pop.Data
	dataLen := len(pop.Data)
	if opcode > txsparser.OP_16 {
...
func removeOpcodeByData(pkscript []txsparser.ParsedOpcode, data []byte) []txsparser.ParsedOpcode {
	retScript := make([]txsparser.ParsedOpcode, 0, len(pkscript))
	for _, pop := range pkscript {
...
// removeOpcodeByData will return the script minus any opcodes that would push
// the passed data to the stack.
func scriptRemoveOpcodeByData(pkscript []scriptInfo, data []byte) []scriptInfo {
	retScript := make([]scriptInfo, 0, len(pkscript))
	for _, pop := range pkscript {
		if !scriptCanonicalPush(pop) || !bytes.Contains(pop.Data, data) {
			retScript = append(retScript, pop)
		}
	}
	return retScript
...
func calcWitnessSignatureHash(subScript []txsparser.ParsedOpcode, sigHashes *TxSigHashes,
	hashType SigHashType, tx *wire.MsgTx, idx int, amt token.TokenValue) ([]byte, error) {
	...
	if txsparser.IsWitnessPubKeyHash(subScript) {
		...
		sigHash.Write([]byte{txsparser.OP_DUP})
		sigHash.Write([]byte{txsparser.OP_HASH160})
		sigHash.Write([]byte{txsparser.OP_DATA_20})
		sigHash.Write(subScript[1].Data)
		sigHash.Write([]byte{txsparser.OP_EQUALVERIFY})
		sigHash.Write([]byte{txsparser.OP_CHECKSIG})
	} else {
		// For p2wsh outputs, and future outputs, the script code is
		// the original script, with all code separators removed,
		// serialized with a var int length prefix.
		rawScript, _ := unparseScript(subScript)
		common.WriteVarBytes(&sigHash, 0, rawScript)
	}
	...
	case *token.NumToken:
		var bAmount [8]byte
		binary.LittleEndian.PutUint64(bAmount[:], uint64(amt.(*token.NumToken).Val))
		sigHash.Write(bAmount[:])
		break
	case *token.HashToken:
		sigHash.Write(amt.(*token.HashToken).Hash[:])
		break
	}
	...
	if hashType&SigHashSingle != SigHashSingle &&
		...
		var b bytes.Buffer
		tx.TxOut[idx].WriteTxOut(&b, 0, 0, wire.BaseEncoding)
...
func CalcWitnessSigHash(script []byte, sigHashes *TxSigHashes, hType SigHashType,
	tx *wire.MsgTx, idx int, amt token.TokenValue) ([]byte, error) {

	parsedScript, err := txsparser.ParseScript(script)
	if err != nil {
...
// engine instance, calculate the signature hash to be used for signing and
// verification.
func calcSignatureHash(script []txsparser.ParsedOpcode, hashType SigHashType, tx *wire.MsgTx, idx int) []byte {
	...
	script = removeOpcode(script, txsparser.OP_CODESEPARATOR)
	...
	case SigHashSingle:
		...
		for i := 0; i < idx; i++ {
			...
			txCopy.TxOut[i].Value = &token.NumToken{Val: -1}
...
func getSigOpCount(pops []txsparser.ParsedOpcode, precise bool) int {
	nSigs := 0
	for i, pop := range pops {
		switch pop.Opcode.Value {
		case txsparser.OP_CHECKSIG:
			fallthrough
		case txsparser.OP_CHECKSIGVERIFY:
			nSigs++
		case txsparser.OP_CHECKMULTISIG:
			fallthrough
		case txsparser.OP_CHECKMULTISIGVERIFY:
			// If we are being precise then look for familiar
			// patterns for multisig, for now all we recognize is
			// OP_1 - OP_16 to signify the number of pubkeys.
			// Otherwise, we use the max of 20.
			if precise && i > 0 &&
				pops[i-1].Opcode.Value >= txsparser.OP_1 &&
				pops[i-1].Opcode.Value <= txsparser.OP_16 {
				nSigs += txsparser.AsSmallInt(pops[i-1].Opcode)
			} else {
...
func GetSigOpCount(script []byte) int {
	...
	// list of pops.
	pops, _ := txsparser.ParseScript(script)
...
func GetPreciseSigOpCount(scriptSig, scriptPubKey []byte, bip16 bool) int {
	// Don't check error since parseScript returns the parsed-up-to-error
	// list of pops.
	pops, _ := txsparser.ParseScript(scriptPubKey)

	// Treat non P2SH transactions as normal.
	if !(bip16 && txsparser.IsScriptHash(pops)) {
	...
	sigPops, err := txsparser.ParseScript(scriptSig)
	if err != nil {
		return 0
	}
	...
	// the case.
	if !isPushOnly(parsedOpcode2scriptInfo(sigPops)) || len(sigPops) == 0 {
		return 0
	}
	...
	// failure.
	shPops, _ := txsparser.ParseScript(shScript)
...
func GetWitnessSigOpCount(sigScript, pkScript []byte, witness wire.TxWitness) int {
	...
	if IsPayToScriptHash(pkScript) && isPushOnly(parsedOpcode2scriptInfo(sigPops)) &&
...
func IsUnspendable(pkScript []byte) bool {
	...
	return len(pops) > 0 && pops[0].Opcode.Value == txsparser.OP_RETURN
}
