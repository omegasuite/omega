...
func isSupportedScriptType(class txsparser.ScriptClass) bool {
	switch class {
	case txsparser.PubKeyHashTy, txsparser.WitnessV0PubKeyHashTy, txsparser.ScriptHashTy, txsparser.ContractHashTy,
		txsparser.WitnessV0ScriptHashTy:
		return true
	default:
		return false
	}
...
func (s PkScript) Script() []byte {
	...
	case txsparser.ContractHashTy:
		script = make([]byte, contractLen)
		copy(script, s.script[:contractLen])
...
func computeNonWitnessPkScript(sigScript []byte) (PkScript, error) {
	...
	case len(sigScript) >= minPubKeyHashSigScriptLen &&
		...
		if btcec.IsCompressedPubKey(pubKey) {
			...
			pkScript := PkScript{class: txsparser.PubKeyHashTy}
	...
	default:
		...
		// obtain it.
		parsedOpcodes, err := txsparser.ParseScript(sigScript)
		if err != nil {
			return PkScript{}, err
		}
		...
		pkScript := PkScript{class: txsparser.ScriptHashTy}
		copy(pkScript.script[:], script)
		return pkScript, nil
	}
}
...
func computeWitnessPkScript(witness wire.TxWitness) (PkScript, error) {
	...
	case len(witness) == 2 && len(lastWitnessItem) == compressedPubKeyLen:
		...
		pkScript.class = txsparser.WitnessV0PubKeyHashTy
		copy(pkScript.script[:], script)
	...
	default:
		...
		pkScript.class = txsparser.WitnessV0ScriptHashTy
		copy(pkScript.script[:], script)
	}
