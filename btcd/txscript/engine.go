...
type Engine struct {
	...
	witnessVersion  int
	witnessProgram  []byte
	inputAmount     token.TokenValue
}

// hasFlag returns whether the script engine instance has the passed flag set.
func (vm *Engine) hasFlag(flag txsparser.ScriptFlags) bool {
...
func (vm *Engine) isBranchExecuting() bool {
	...
	return vm.condStack[len(vm.condStack)-1] == txsparser.OpCondTrue
}

// executeOpcode peforms execution on the passed opcode.  It takes into account
// whether or not it is hidden by conditionals, but some rules still must be
// tested in this case.
func (vm *Engine) executeOpcode(pop *scriptInfo) error {
	...
	if pop.Opcode.Value > txsparser.OP_16 {
		vm.numOps++
		...
		return txsparser.ScriptError(txsparser.ErrElementTooBig, str)
	}
	...
	if vm.dstack.verifyMinimalData && vm.isBranchExecuting() &&
		pop.Opcode.Value >= 0 && pop.Opcode.Value <= txsparser.OP_PUSHDATA4 {
	...
	return pop.opfunc(&pop.ParsedOpcode, vm)
}
...
func (vm *Engine) validPC() error {
	if vm.scriptIdx >= len(vm.scripts) {
		...
		return txsparser.ScriptError(txsparser.ErrInvalidProgramCounter, str)
	}
	...
	if vm.isWitnessVersionActive(0) {
		...
		case payToWitnessPubKeyHashDataSize: // P2WKH
			...
			if len(witness) != 2 {
				...
				return txsparser.ScriptError(txsparser.ErrWitnessProgramMismatch, err)
			}
			...
			// script to execute.
			vm.scripts = append(vm.scripts, parsedOpcode2scriptInfo(pops))
			vm.SetStack(witness)
		...
		case payToWitnessScriptHashDataSize: // P2WSH
			...
			// this point.
			if len(witness) == 0 {
				return txsparser.ScriptError(txsparser.ErrWitnessProgramEmpty, "witness "+
			...
			if len(witnessScript) > MaxScriptSize {
				...
				return txsparser.ScriptError(txsparser.ErrScriptTooBig, str)
			}
			...
			if !bytes.Equal(witnessHash[:], vm.witnessProgram) {
				return txsparser.ScriptError(txsparser.ErrWitnessProgramMismatch,
			...
			// as the next script.
			pops, err := txsparser.ParseScript(witnessScript)
			if err != nil {
				return err
			}
		...
		default:
			...
			return txsparser.ScriptError(txsparser.ErrWitnessProgramWrongLength, errStr)
		}
	} else if vm.hasFlag(txsparser.ScriptVerifyDiscourageUpgradeableWitnessProgram) {
		errStr := fmt.Sprintf("new witness program versions "+
			"invalid: %v", vm.witnessProgram)
		return txsparser.ScriptError(txsparser.ErrDiscourageUpgradableWitnessProgram, errStr)
	} else {
...
func (vm *Engine) DisasmScript(idx int) (string, error) {
	if idx >= len(vm.scripts) {
		str := fmt.Sprintf("script index %d >= total scripts %d", idx,
			len(vm.scripts))
		return "", txsparser.ScriptError(txsparser.ErrInvalidIndex, str)
	}
...
func (vm *Engine) CheckErrorCondition(finalScript bool) error {
	...
	if vm.scriptIdx < len(vm.scripts) {
		return txsparser.ScriptError(txsparser.ErrScriptUnfinished,
	...
	if finalScript && vm.isWitnessVersionActive(0) && vm.dstack.Depth() != 1 {
		return txsparser.ScriptError(txsparser.ErrEvalFalse, "witness program must "+
			"have clean stack")
	}

	if finalScript && vm.hasFlag(txsparser.ScriptVerifyCleanStack) &&
		vm.dstack.Depth() != 1 {

		str := fmt.Sprintf("stack contains %d unexpected items",
			vm.dstack.Depth()-1)
		return txsparser.ScriptError(txsparser.ErrCleanStack, str)
	} else if vm.dstack.Depth() < 1 {
		return txsparser.ScriptError(txsparser.ErrEmptyStack,
	...
	if !v {
		...
		return txsparser.ScriptError(txsparser.ErrEvalFalse,
...
func (vm *Engine) Step() (done bool, err error) {
	...
	if combinedStackSize > MaxStackSize {
		...
		return false, txsparser.ScriptError(txsparser.ErrStackOverflow, str)
	}
	...
	if vm.scriptOff >= len(vm.scripts[vm.scriptIdx]) {
		...
		if err == nil && len(vm.condStack) != 0 {
			return false, txsparser.ScriptError(txsparser.ErrUnbalancedConditional,
		...
		if vm.scriptIdx == 0 && vm.bip16 {
			...
			pops, err := txsparser.ParseScript(script)
			if err != nil {
				return false, err
			}
...
func (vm *Engine) subScript() []scriptInfo {
...
func (vm *Engine) checkHashTypeEncoding(hashType SigHashType) error {
	...
	if sigHashType < SigHashAll || sigHashType > SigHashSingle {
		...
		return txsparser.ScriptError(txsparser.ErrInvalidSigHashType, str)
	}
	return nil
}
...
func (vm *Engine) checkPubKeyEncoding(pubKey []byte) error {
	if vm.hasFlag(txsparser.ScriptVerifyWitnessPubKeyType) &&
		vm.isWitnessVersionActive(0) && !btcec.IsCompressedPubKey(pubKey) {

		str := "only uncompressed keys are accepted post-segwit"
		return txsparser.ScriptError(txsparser.ErrWitnessPubKeyType, str)
	}
	...
	return txsparser.ScriptError(txsparser.ErrPubKeyType, "unsupported public key type")
}

// checkSignatureEncoding returns whether or not the passed signature adheres to
// the strict encoding requirements if enabled.
func (vm *Engine) checkSignatureEncoding(sig []byte) error {
	if !vm.hasFlag(txsparser.ScriptVerifyDERSignatures) &&
		!vm.hasFlag(txsparser.ScriptVerifyLowS) &&
		!vm.hasFlag(txsparser.ScriptVerifyStrictEncoding) {
	...
	if sigLen < minSigLen {
		str := fmt.Sprintf("malformed signature: too short: %d < %d", sigLen,
			minSigLen)
		return txsparser.ScriptError(txsparser.ErrSigTooShort, str)
	}
	if sigLen > maxSigLen {
		str := fmt.Sprintf("malformed signature: too long: %d > %d", sigLen,
			maxSigLen)
		return txsparser.ScriptError(txsparser.ErrSigTooLong, str)
	}
	...
	if sig[sequenceOffset] != asn1SequenceID {
		str := fmt.Sprintf("malformed signature: format has wrong type: %#x",
			sig[sequenceOffset])
		return txsparser.ScriptError(txsparser.ErrSigInvalidSeqID, str)
	}
	...
	if int(sig[dataLenOffset]) != sigLen-2 {
		...
		return txsparser.ScriptError(txsparser.ErrSigInvalidDataLen, str)
	}
	...
	if sTypeOffset >= sigLen {
		...
		return txsparser.ScriptError(txsparser.ErrSigMissingSTypeID, str)
	}
	if sLenOffset >= sigLen {
		str := "malformed signature: S length missing"
		return txsparser.ScriptError(txsparser.ErrSigMissingSLen, str)
	}
	...
	if sOffset+sLen != sigLen {
		...
		return txsparser.ScriptError(txsparser.ErrSigInvalidSLen, str)
	}
	...
	if sig[rTypeOffset] != asn1IntegerID {
		...
		return txsparser.ScriptError(txsparser.ErrSigInvalidRIntID, str)
	}

	// Zero-length integers are not allowed for R.
	if rLen == 0 {
		str := "malformed signature: R length is zero"
		return txsparser.ScriptError(txsparser.ErrSigZeroRLen, str)
	}

	// R must not be negative.
	if sig[rOffset]&0x80 != 0 {
		str := "malformed signature: R is negative"
		return txsparser.ScriptError(txsparser.ErrSigNegativeR, str)
	}
	...
	if rLen > 1 && sig[rOffset] == 0x00 && sig[rOffset+1]&0x80 == 0 {
		...
		return txsparser.ScriptError(txsparser.ErrSigTooMuchRPadding, str)
	}
	...
	if sig[sTypeOffset] != asn1IntegerID {
		...
		return txsparser.ScriptError(txsparser.ErrSigInvalidSIntID, str)
	}

	// Zero-length integers are not allowed for S.
	if sLen == 0 {
		str := "malformed signature: S length is zero"
		return txsparser.ScriptError(txsparser.ErrSigZeroSLen, str)
	}

	// S must not be negative.
	if sig[sOffset]&0x80 != 0 {
		str := "malformed signature: S is negative"
		return txsparser.ScriptError(txsparser.ErrSigNegativeS, str)
	}
	...
	if sLen > 1 && sig[sOffset] == 0x00 && sig[sOffset+1]&0x80 == 0 {
		...
		return txsparser.ScriptError(txsparser.ErrSigTooMuchSPadding, str)
	}
	...
	if vm.hasFlag(txsparser.ScriptVerifyLowS) {
		sValue := new(big.Int).SetBytes(sig[sOffset : sOffset+sLen])
		if sValue.Cmp(halfOrder) > 0 {
			return txsparser.ScriptError(txsparser.ErrSigHighS, "signature is not canonical due "+
...
func NewEngine(scriptPubKey []byte, tx *wire.MsgTx, txIdx int, flags txsparser.ScriptFlags,
	sigCache *SigCache, hashCache *TxSigHashes, inputAmount token.TokenValue) (*Engine, error) {

	// The provided transaction input index must refer to a valid input.
	if txIdx < 0 || txIdx >= len(tx.TxIn) {
		str := fmt.Sprintf("transaction input index %d is negative or "+
			">= %d", txIdx, len(tx.TxIn))
		return nil, txsparser.ScriptError(txsparser.ErrInvalidIndex, str)
	}
	...
	if len(scriptSig) == 0 && len(scriptPubKey) == 0 {
		return nil, txsparser.ScriptError(txsparser.ErrEvalFalse,
			"false stack entry at end of script execution")
	}
	...
	if vm.hasFlag(txsparser.ScriptVerifyCleanStack) && (!vm.hasFlag(txsparser.ScriptBip16) &&
		!vm.hasFlag(txsparser.ScriptVerifyWitness)) {
		return nil, txsparser.ScriptError(txsparser.ErrInvalidFlags,
			"invalid flags combination")
	}

	// The signature script must only contain data pushes when the
	// associated flag is set.
	if vm.hasFlag(txsparser.ScriptVerifySigPushOnly) && !IsPushOnlyScript(scriptSig) {
	...
	vm.scripts = make([][]scriptInfo, len(scripts))
	for i, scr := range scripts {
		if len(scr) > MaxScriptSize {
			str := fmt.Sprintf("script size %d is larger than max "+
				"allowed size %d", len(scr), MaxScriptSize)
			return nil, txsparser.ScriptError(txsparser.ErrScriptTooBig, str)
		}
		var err error
		pops, err := txsparser.ParseScript(scr)
		if err != nil {
			return nil, err
		}

		vm.scripts[i] = parsedOpcode2scriptInfo(pops)
	}
	...
	if vm.hasFlag(txsparser.ScriptBip16) && txsparser.IsScriptHash(scriptInfo2parsedOpcode(vm.scripts[1])) {
	...
	if vm.hasFlag(txsparser.ScriptVerifyMinimalData) {
	...
	if vm.hasFlag(txsparser.ScriptVerifyWitness) {
		// If witness evaluation is enabled, then P2SH MUST also be
		// active.
		if !vm.hasFlag(txsparser.ScriptBip16) {
			errStr := "P2SH must be enabled to do witness verification"
			return nil, txsparser.ScriptError(txsparser.ErrInvalidFlags, errStr)
		}
		...
		case isWitnessProgram(vm.scripts[1]):
			...
			if len(scriptSig) != 0 {
				...
				return nil, txsparser.ScriptError(txsparser.ErrWitnessMalleated, errStr)
			}
		...
		case len(tx.TxIn[txIdx].Witness) != 0 && vm.bip16:
			...
			if len(sigPops) == 1 && canonicalPush(sigPops[0].ParsedOpcode) &&
				...
				return nil, txsparser.ScriptError(txsparser.ErrWitnessMalleatedP2SH, errStr)
			}
		}
		...
		if witProgram != nil {
			...
			if vm.witnessProgram == nil && len(tx.TxIn[txIdx].Witness) != 0 {
				...
				return nil, txsparser.ScriptError(txsparser.ErrWitnessUnexpected, errStr)
			}
		}
