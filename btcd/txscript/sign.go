...
func RawTxInWitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int,
	amt  token.TokenValue, subScript []byte, hashType SigHashType,
	key *btcec.PrivateKey) ([]byte, error) {

	parsedScript, err := txsparser.ParseScript(subScript)
	if err != nil {
...
func WitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int, amt  token.TokenValue,
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	...
	builder := NewScriptBuilder().AddOp(txsparser.OP_FALSE)
	signed := 0
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	txsparser.ScriptClass, []btcutil.Address, int, error) {

	class, addresses, nrequired, err := ExtractPkScriptAddrs(subScript,
		chainParams)
	if err != nil {
		return nil, txsparser.NonStandardTy, nil, 0, err
	}
...
func mergeScripts(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	pkScript []byte, class txsparser.ScriptClass, addresses []btcutil.Address,
	...
	case txsparser.ScriptHashTy:
		...
		prevPops, err := txsparser.ParseScript(prevScript)
...
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []btcutil.Address,
	...
	pkPops, _ := txsparser.ParseScript(pkScript)
	...
	extractSigs := func(pops []txsparser.ParsedOpcode, sigs [][]byte) [][]byte {
