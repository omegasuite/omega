...
// GetAssetCmd defines the getasset JSON-RPC command.
type GetAssetCmd struct {
	Account *string
	Detail  *bool `jsonrpcdefault:false`
}

// NewGetAssetCmd returns a new instance which can be used to issue a
// getasset JSON-RPC command.
//
func NewGetAssetCmd(account *string) *GetAssetCmd {
	return &GetAssetCmd{
		Account: account,
	}
}
...
// getasset JSON-RPC command.
//
func NewGetDBCmd(bucket *string) *GetDBCmd {
	return &GetDBCmd{
		Bucket: bucket,
	}
}
...
type SendManyCmd struct {
	FromAccount string
	Amounts     map[string]float64 `jsonrpcusage:"{\"address\":amount,...}"` // In OMC
	MinConf     *int               `jsonrpcdefault:"1"`
	Comment     *string
}
...
// The parameters which are pointers indicate they are optional.  Passing nil
// for optional parameters will use the default value.
func NewSignRawTransactionCmd(hexEncodedTx string, inputs *[]RawTxInput, privKeys *[]string, flags *string, hash bool) *SignRawTransactionCmd {
...
func init() {
	...
	MustRegisterCmd("getasset", (*GetAssetCmd)(nil), flags)
	MustRegisterCmd("getdb", (*GetDBCmd)(nil), flags)
