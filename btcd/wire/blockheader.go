...
const (
	MaxCommitteeSize = 3
	MinerGap         = 3  // a miner must wait between two candidacies. should >= CommitteeSize - 1
...
const (
	...
	ChgParam = InstructionCode(2)

	// these code are used by BOVM only, we include here to prevent ambiguity
	AddRedeemScript = InstructionCode(3)
	DelRedeemScript = InstructionCode(4)
	Pledge          = InstructionCode(5)
	RetireReq       = InstructionCode(6)
)
...
type MingingRightBlock struct {
	...
	ContractLimit   int32         // alternative contract execution limit
