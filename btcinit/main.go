...
func main() {
	...
	fmt.Printf("\n\nvar coinToken = token.Token{\n\t"+
		"TokenType: 0,\n\t"+
		"Value: &token.NumToken{Val: %s},\n\t"+
		"Rights: &chainhash.Hash{},\n}", blockchain.BaseSubsidy)
	...
	*/
	...
	for net, k := range addresses {
		...
		genesisCoinbaseTx.TxOut[0].Value = &token.NumToken{Val: blockchain.BaseSubsidy}
		...
		merkles := blockchain.BuildMerkleTreeStore([]*btcutil.Tx{t1, t2}, false, wire.Version1)

		// genesisMerkleRoot is the hash of the first transaction in the genesis block
		// for the main network.
		var genesisMerkleRoot = merkles[len(merkles)-1]

		witnessMerkleTree := blockchain.BuildMerkleTreeStore([]*btcutil.Tx{t1, t2}, true, wire.Version1)
		...
		fmt.Printf("},"+
			...
			"\n\tContractLimit: %d,"+
			"\n\tMeanTPH: %d\n}", minerBlock.ContractLimit, minerBlock.MeanTPH)
	}
}
