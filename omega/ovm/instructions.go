...
func opSload(pc *uint64, evm *OVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	val := evm.StateDB.GetState(&loc).Big()
	stack.push(val)
	return nil, nil
}
...
func opGas(pc *uint64, evm *OVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
...
// new ops
func opGetTxTemplate(pc *uint64, evm *OVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	tx := evm.GetTxTemplate()
	n := uint64(16)
	for _,def := range tx.TxDef {
		// deftype, hash, father, attrib, len of desc
		n += 70 + uint64(len(def.(*token.RightDef).Desc))
	}
	...
	p += 4
	memory.SetUint32(p, uint32(len(tx.TxDef)))
	p += 4
	for _,def := range tx.TxDef {
		memory.SetByte(p, def.DefType())
		p++
		d := def.(*token.RightDef)
		memory.Set(p, 32, def.Hash()[:])
		p += 32
		memory.Set(p, 32, d.Father[:])
		p += 32
		memory.SetByte(p, d.Attrib)
		p++
		memory.SetUint32(p, uint32(len(d.Desc)))
		p += 4
		memory.Set(p, uint64(len(d.Desc)), d.Desc[:])
		p += uint64(len(d.Desc))
	}
	...
	p += 4
	for _,t := range tx.TxOut {
		memory.SetUint64(p, t.TokenType)
		p += 8
		memory.SetUint64(p, uint64(t.Token.Value.(*token.NumToken).Val))
		p += 8
		if t.TokenType & 2 != 0 {
			memory.SetUint32(p, uint32(len(t.Rights)))
			p += 4
	...
	return memory.GetPtr(int64(ptr), int64(n)), nil
}
...
func opSubmit(pc *uint64, evm *OVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
}

func opGetDefinition(pc *uint64, evm *OVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	evm.chainConfig.HomesteadBlock
	tx := evm.GetTxTemplate()
	n := uint64(16);
	for _,def := range tx.TxDef {
		// deftype, hash, father, attrib, len of desc
